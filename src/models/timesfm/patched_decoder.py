...
class PatchedTimeSeriesDecoder(base_layer.BaseLayer):
    ...
    stacked_transformer_params_tpl: LayerTpl = template_field(transformers.StackedTransformer)
    use_freq: bool = True
    ...
    def setup(self) -> None:
        ...
        self.create_child(
            "position_emb",
            pax_fiddle.Config(layers.PositionalEmbedding, embedding_dims=self.model_dims),
        )
    ...
    def _forward_transform(self, inputs: JTensor, patched_pads: JTensor) -> Tuple[JTensor, Tuple[JTensor, JTensor]]:
    ...
    def _reverse_transform(self, outputs: JTensor, stats: Tuple[JTensor, JTensor]) -> JTensor:
    ...
    def _preprocess_input(
        ...
        input_padding = jnp.where(jnp.abs(input_ts - PAD_VAL) < _TOLERANCE, 1, input_padding)
    ...
    def _postprocess_output(
        ...
        output_ts = es.jax_einshape("bn(hq)->bnhq", output_ts, q=num_outputs, h=self.horizon_len)
    ...
    def __call__(self, inputs: NestedMap) -> NestedMap:
        ...
        return NestedMap({_OUTPUT_TOKENS: model_output, _OUTPUT_TS: output_ts, _STATS: stats})
