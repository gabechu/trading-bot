...
from src.backend.models.timesfm.patched_decoder import DEFAULT_QUANTILES, PatchedTimeSeriesDecoder, ResidualBlock
...
class TimesFm:
    ...
    def __init__(
        ...
        self.model_p = pax_fiddle.Config(
            ...
            residual_block_tpl=pax_fiddle.Config(ResidualBlock),
            quantiles=quantiles,
            use_freq=True,
    ...
    def load_from_checkpoint(
        ...
        var_weight_hparams = self._model.abstract_init_with_metadata(self._get_sample_inputs(), do_eval=True)
        ...
        self._logging(f"Constructed model weights in {time.time() - start_time:.2f} seconds.")
    ...
    def forecast(
        ...
        if not self._train_state or not self._model:
            raise ValueError("Checkpoint not loaded. Call `load_from_checkpoint` before" " `forecast`.")
        ...
        with base_layer.JaxContext.new_context(hparams=self._eval_context):
            ...
            for i in range(input_ts.shape[0] // self.global_batch_size):
                input_ts_in = jnp.array(input_ts[i * self.global_batch_size : (i + 1) * self.global_batch_size])
                input_padding_in = jnp.array(
                    input_padding[i * self.global_batch_size : (i + 1) * self.global_batch_size],
                )
                inp_freq_in = jnp.array(
                    inp_freq[i * self.global_batch_size : (i + 1) * self.global_batch_size, :],
                    dtype=jnp.int32,
                )
                ...
                mean_output = es.jax_einshape("db...->(db)...", mean_output, d=self.num_devices)
                full_output = es.jax_einshape("db...->(db)...", full_output, d=self.num_devices)
    ...
    def forecast_on_df(
        ...
        """
        if not ("unique_id" in inputs.columns and "ds" in inputs.columns and value_name in inputs.columns):
            raise ValueError(f"DataFrame must have unique_id, ds and {value_name} columns.")
        ...
        if num_jobs == 1:
            ...
            with multiprocessing.Pool(processes=num_jobs) as pool:
                results = pool.starmap(
                    process_group,
                    [(key, group, value_name, forecast_context_len) for key, group in df_sorted.groupby("unique_id")],
                )
            new_inputs, uids = zip(*results)
        print("Finished preprocessing dataframe.")
        freq_inps = [freq_map(freq)] * len(new_inputs)
        _, full_forecast = self.forecast(new_inputs, freq=freq_inps, window_size=window_size)
        ...
        if self._model.quantiles is not None:
            for i, q in enumerate(self._model.quantiles):
                ...
                fcst_df[q_col] = full_forecast[:, 0 : self.horizon_len, 1 + i].reshape(-1, 1)
                if q == 0.5:
